...
用法:
    python scripts/plot_training_log.py [日志文件路径] [输出图片路径] [起始步数]

示例:
    python scripts/plot_training_log.py nohup_log.txt training_curves.png 1000
    python scripts/plot_training_log.py nohup_log.txt training_curves.png 0
...
def parse_log(log_file):
    """解析日志文件，提取 step、loss、validation loss 和 HellaSwag accuracy。"""
    steps = []
    losses = []
...
def plot_training_curves(data, output_file="training_curves.png", start_step=1000):
    ...
    if data["hellaswag_steps"]:
        ax3.plot(
            data["hellaswag_steps"],
            data["hellaswag_accs"],
            "g-o",
            linewidth=2,
            markersize=6,
            label="HellaSwag Accuracy",
        )
    ...
    if data["train_losses"]:
        print(
            f"训练Loss - 最小: {min(data['train_losses']):.4f}, "
            f"最大: {max(data['train_losses']):.4f}, 最终: {data['train_losses'][-1]:.4f}"
        )
    print(f"验证次数: {len(data['val_steps'])}")
    if data["val_losses"]:
        print(
            f"验证Loss - 最小: {min(data['val_losses']):.4f}, "
            f"最大: {max(data['val_losses']):.4f}, 最终: {data['val_losses'][-1]:.4f}"
        )
    print(f"HellaSwag评估次数: {len(data['hellaswag_steps'])}")
    if data["hellaswag_accs"]:
        print(
            f"HellaSwag准确率 - 最小: {min(data['hellaswag_accs']):.4f}, "
            f"最大: {max(data['hellaswag_accs']):.4f}, 最终: {data['hellaswag_accs'][-1]:.4f}"
        )
